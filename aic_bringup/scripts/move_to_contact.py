...
class ControlReferencePublisher(Node):
    def __init__(self):
        ...
        self.controller_namespace = (
            self.get_parameter("controller_namespace")
            .get_parameter_value()
            .string_value
        )
        self.tool_frame = (
        ...
        self.contact_force_z = (
            self.get_parameter("contact_force_z").get_parameter_value().double_value
        )
    ...
    def generate_joint_ref(
        self, positions=[], velocities=[], accelerations=[], effort=[]
    ):
